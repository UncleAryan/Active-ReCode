...
import { getFsrsCard } from "./scheduler.js";
...
describe("Scheduler", () => {
   ...
   describe("createCard", () => {
    ...
    it("Creates a new card if one doesn't exist", async () => {
        ...
        expect(storage.addCard).toHaveBeenCalledWith(userId, challengeId, expect.any(Object));
    ...
    describe("review", () => {
        ...
        // Tests taking the users input and updating the cards dificulty based on the users rating
            it("updates the cards based on user's review rating", async () => {
                ...
                vi.spyOn(scheduler, "getFsrsCard").mockResolvedValue({ id: "Outdated-Card" });
                ...
                expect(_mockScheduler.next).toHaveBeenCalledWith({ id: "Outdated-Card" }, now, Rating.Hard);

                expect(storage.updateCard).toHaveBeenCalledWith(userId, challengeId, fsrsResult.card);
